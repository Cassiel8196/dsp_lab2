...
def dtw_re(test, ref, i, j):  # testframe和refframe分别是两个音频单帧的mfcc数组
    if i == 0 & j == 0:
        return 0
    elif i == 0:
        _d = abs(test[i] - ref[j])
        return _d + dtw_re(test, ref, i, j - 1)
    elif j == 0:
        _d = abs(test[i] - ref[j])
        return _d + dtw_re(test, ref, i - 1, j)
    else:
        _d = abs(test[i] - ref[j])
        return _d + min(dtw_re(test, ref, i - 1, j), dtw_re(test, ref, i, j - 1), dtw_re(test, ref, i - 1, j - 1))
...
def main():
    ...
    # 下面这行是接口，test是待识别音频的mfcc数组（它真的是一维的），ref是参考音频的mfcc数组
    # print(dtw_re(test, ref, len(test) - 1, len(ref) - 1))
